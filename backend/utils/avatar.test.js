...
describe("avatar utilities", () => {
  ...
  it("falls back to the default template when no env override is provided", () => {
    ...
    expect(url).toBe("https://avatar.iran.liara.run/public/boy?username=john%20doe");
  });

  it("supports custom templates with placeholders", () => {
    process.env.AVATAR_TEMPLATE_URL = "https://cdn.example.com/{{gender}}/{{genderPath}}/{{username}}";
  ...
  it("returns existing profile pics via ensureAvatarUrl", () => {
    const profilePic = ensureAvatarUrl({ username: "abc", gender: "male", profilePic: "https://existing" });
    expect(profilePic).toBe("https://existing");
  });

  it("builds a new url when profile pic is empty", () => {
    const profilePic = ensureAvatarUrl({ username: "abc", gender: "female", profilePic: "" });
    expect(profilePic).toBe("https://avatar.iran.liara.run/public/girl?username=abc");
  });
});
